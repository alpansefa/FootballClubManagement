  ...
  supprimer(id: number) {
    ...
    this.us.deleteJoueur(id as number)
      .pipe(
        ...
        finalize(() => this.toastr.success("le joueur a été supprimé avec succès", "Gestion des joueurs", { positionClass: 'toast-bottom-right' }))
    ...
    if (this.registerForm.valid) {
      ...
      this.us.addJoueur(joueur)
        .pipe(
          ...
          mergeMap(() => this.us.getJoueursFromClub(this.user.club_id as number)),
          finalize(() => this.toastr.success(joueur.prenom + " " + joueur.nom + " a été ajouté", "Gestion des joueurs", { positionClass: 'toast-bottom-right' }))
        )
        .subscribe((datas) => {
          this.joueurs = datas;
          this.registerForm.reset();
          this.activForm = false;
          // this.snackBarService.openSnackBar(joueur.prenom as string + ' ' + joueur.nom as string + " a été ajouté");
          // this.snackBarService.showSnackbarAction(joueur.prenom as string + ' ' + joueur.nom as string+ " a été ajouté");
        })
    }
  }
    ...
    if (this.editForm.valid) {
      ...
      this.us.updateJoueur(this.id, joueur)
        .pipe(
          ...
          mergeMap(() => this.us.getJoueursFromClub(this.user.club_id as number)),
          finalize(() => this.toastr.success(joueur.prenom as string + ' ' + joueur.nom as string + " a été modifié avec succès", "Gestion des joueurs", {positionClass:"toast-bottom-right"}))
