  ...
  supprimer(id: number) {
    this.es.deleteEquipe(id as number)
      .pipe(
        ...
        finalize(()=> this.toastr.success("L'équipe a été supprimée", "Gestion des équipes", {positionClass: 'toast-bottom-right' }))
      ...
      if (this.editForm) {
        this.es.updateEquipe(this.equipe.id, equipe)
          .pipe(
            ...
            finalize(()=> this.toastr.success(equipe.categorie+" a été modifiée", "Gestion des équipes", {positionClass: 'toast-bottom-right' }))
          )
          .subscribe(datas => {
            this.equipes = datas;
        ...
        this.es.addEquipe(equipe)
          .pipe(
            mergeMap(() => this.es.getEquipesClub(this.president.club_id as number)),
            finalize(()=> this.toastr.success(equipe.categorie + " a été ajouté", "Gestion des équipes", {positionClass: 'toast-bottom-right' }))
          )
          .subscribe(datas => {
            this.equipes = datas;
            this.activForm = false;
